...
func (p *namespacedCloudProfile) Mutate(ctx context.Context, newObj, _ client.Object) error {
	...
	// Mutation 1: Populate capabilityFlavors on spec.machineImages
	if err := p.mutateSpecCapabilityFlavors(ctx, profile); err != nil {
		return err
	}

	// Mutation 2: Merge spec providerConfig into status (only on status subresource updates)
	if err := p.mergeStatusProviderConfig(profile); err != nil {
		return err
	}
...
// mutateSpecCapabilityFlavors populates capabilityFlavors on spec.machineImages based on the
// provider config. This ensures Gardener core validation sees capabilityFlavors before it runs.
func (p *namespacedCloudProfile) mutateSpecCapabilityFlavors(ctx context.Context, profile *gardencorev1beta1.NamespacedCloudProfile) error {
	if profile.Spec.Parent.Name == "" || len(profile.Spec.MachineImages) == 0 || profile.Spec.ProviderConfig == nil {
		return nil
	}
...
// This only runs on status subresource updates (when generation == observedGeneration).
func (p *namespacedCloudProfile) mergeStatusProviderConfig(profile *gardencorev1beta1.NamespacedCloudProfile) error {
...
// shouldMergeStatus returns true when the status has been populated by the reconciler
// (generation matches observedGeneration) and both configs are available.
